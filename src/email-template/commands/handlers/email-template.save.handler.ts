...
import { EmailTemplateEnum, IEmailTemplate } from '../../../models';
  ...
  implements ICommandHandler<EmailTemplateSaveCommand>
{
  ...
  public async execute(
    ...
    const {
      input: { name, mjml, subject },
    } = command;
    try {
      await this._saveTemplate(name, mjml, 'html');
    } catch (error) {
      // TODO add translation
      throw new BadRequestException('Invalid html template');
    }

    return this._saveTemplate(name, subject, 'subject');
  }
