...
import { IPluginConfig, isNotEmpty } from '../common';
  ...
  try {
    ...
    if (isNotEmpty(migrations)) {
      for (const migration of migrations) {
        console.log(
          chalk.green(`Migration ${migration.name} has been run successfully!`),
        );
      }
    } else {
...
export async function generateMigration(
  ...
  if (!options.name) {
    console.log(
      chalk.yellow(
        'Migration name must be required.Please specify migration name!',
      ),
    );
    return;
  }
  ...
  try {
    ...
    if (upSqls.length) {
      ...
      try {
        ...
        console.log(
          chalk.black.bgRed('Error during migration generating files:'),
        );
        console.error(error);
      }
    } else {
...
export async function createMigration(
  ...
  try {
    ...
    try {
      ...
      console.log(
        chalk.green(
          `Migration ${chalk.blue(outputPath)} has been created successfully.`,
        ),
      );
    } catch (error) {
...
export class ${camelCase(
    ...
    */
    public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${
          ['sqlite', 'better-sqlite3'].includes(connection.options.type)
            ? upSqls.join(`
        `)
            : [].join(`
        `)
