...
import { ICloudinaryConfig, isNotEmpty } from '../../../common';
...
export class CloudinaryProvider extends Provider<CloudinaryProvider> {
  ...
  private getCloudinaryInstance(): ConfigOptions {
    try {
      ...
      console.log(
        `Error while retrieving ${FileStorageProviderEnum.CLOUDINARY} instance:`,
        error,
      );
    }
  }
  ...
  handler({
    ...
    return new CloudinaryStorage({
      ...
      params: (_req: Request, file: Express.Multer.File) => {
        ...
        if (filename) {
          public_id =
            typeof filename === 'string' ? filename : filename(file, format);
        } else {
          public_id = `${prefix}-${moment().unix()}-${parseInt(
            '' + Math.random() * 1000,
            10,
          )}`;
        }
  ...
  url(fileURL: string): string {
    ...
    return new URL(
      join(this.config.cloud_name, fileURL),
      this.config.baseUrl,
    ).toString();
  }
  ...
  path(filePath: string): string {
    ...
    return new URL(
      join(this.config.cloud_name, filePath),
      this.config.baseUrl,
    ).toString();
  }
  ...
  /**
   * Map uploaded file for cloudinary providers
   *
   * @param file
   * @returns
   */
